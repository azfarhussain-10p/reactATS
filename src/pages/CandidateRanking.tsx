...
const CandidateRanking = () => {
  ...
  const [rankedCandidates, setRankedCandidates] = useState<Array<{
    candidateId: number;
    score: number;
    ranking: number;
    candidate?: Candidate;
  ...
  const handleWeightChange = (name: keyof typeof rankingWeights) => (event: any, newValue: number | number[]) => {
  ...
  const getFilteredCandidates = () => {
    ...
    if (requiredSkillFilter) {
      filtered = filtered.filter(c => {
        // This would need proper implementation based on your data structure
        const candidateSkills = c.candidate?.professionalInfo.skills.map(s => s.name.toLowerCase()) || [];
  ...
  const hasDuplicates = (candidateId: number) => {
    return duplicates.some(d => 
      d.candidateId === candidateId || d.duplicateIds.includes(candidateId)
    );
  };
                  ...
                  <TableBody>
                    {getFilteredCandidates().map((candidate) => (
                      <TableRow 
                        ...
                        sx={{ 
                          cursor: 'pointer',
                          bgcolor: selectedCandidateId === candidate.candidateId ? 'action.selected' : 'inherit',
                      ...
                      >
                        <TableCell>
                          <Chip 
                            ...
                            color={
                              candidate.ranking <= 3 ? 'success' : 
                              candidate.ranking <= 10 ? 'primary' : 
                              'default'
                            }
                            size="small"
                          />
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            <Box>
                              <Typography variant="body2">
                                {candidate.candidate?.personalInfo.firstName} {candidate.candidate?.personalInfo.lastName}
                        ...
                        <TableCell>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <LinearProgress 
                              ...
                              sx={{ width: 100 }}
                              color={
                                candidate.score >= 0.8 ? 'success' : 
                                candidate.score >= 0.6 ? 'primary' : 
                                candidate.score >= 0.4 ? 'warning' : 
                                'error'
                              }
                            />
                            <Typography variant="body2">
                    ...
                    <Stack spacing={2}>
                      {duplicates.map((dup, index) => (
                        <Alert key={index} severity="warning" icon={<CompareIcon />}>
                          <Typography variant="body2">
                            Candidate #{dup.candidateId} may be a duplicate of {dup.duplicateIds.join(', ')}
                          </Typography>
                          <Typography variant="caption">
