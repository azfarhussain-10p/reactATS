...
class PerformanceMonitorService {
  ...
  private constructor() {
    ...
    this.reportingEndpoint = import.meta.env.VITE_PERFORMANCE_ENDPOINT || '/api/metrics/performance';
      ...
      if ('PerformanceObserver' in window) {
        const observer = new PerformanceObserver((list) => {
          for (const entry of list.getEntries()) {
            this.recordMetric('render', 'First Contentful Paint', {
   ...
   */
  public startMetric(type: PerformanceMetric['type'], name: string, metadata?: Record<string, any>): string {
  ...
  private recordPageLoad(): void {
    ...
    this.recordMetric('page_load', window.location.pathname, {
    ...
    }, perfData.navigationStart, perfData.loadEventEnd);
  }
  
  /**
   * Wrapper for measuring search query performance
   */
  public async measureSearch<T>(searchFn: () => Promise<T>, queryInfo: Record<string, any>): Promise<T> {
  ...
  private isAboveThreshold(metric: PerformanceMetric): boolean {
    ...
    switch (metric.type) {
      ...
      case 'bulk_operation':
        ...
        const threshold = PERFORMANCE_TARGETS.BULK_OPERATION_BASE_TIME_MS + 
