...
interface EmailCampaignContextType {
  ...
  createCampaign: (campaign: Omit<EmailCampaign, 'id' | 'steps' | 'createdAt' | 'updatedAt' | 'stats'>) => EmailCampaign;
  ...
  addCampaignStep: (campaignId: string, step: Omit<EmailCampaignStep, 'id' | 'campaignId'>) => EmailCampaignStep;
  updateCampaignStep: (stepId: string, updates: Partial<EmailCampaignStep>) => EmailCampaignStep | undefined;
  ...
  updateEmailLogStatus: (id: string, status: EmailLog['status'], details?: Partial<EmailLog>) => EmailLog | undefined;
  
  // Email Operations
  previewEmail: (templateId: string, candidateId: number, jobId?: number) => { subject: string, body: string };
  sendTestEmail: (templateId: string, to: string, candidateId?: number, jobId?: number) => Promise<boolean>;
  processEmailTriggers: (trigger: EmailTrigger, candidateId: number, specifics?: any) => Promise<number>; // Returns number of emails sent
...
export const EmailCampaignProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const updatedCampaigns = campaigns.map(campaign => {
      const steps = campaignSteps.filter(step => step.campaignId === campaign.id)
  ...
  const createCampaign = (campaign: Omit<EmailCampaign, 'id' | 'steps' | 'createdAt' | 'updatedAt' | 'stats'>) => {
  ...
  const addCampaignStep = (campaignId: string, step: Omit<EmailCampaignStep, 'id' | 'campaignId'>) => {
    ...
    if (order <= existingSteps.length) {
      setCampaignSteps(prev => 
        prev.map(s => 
          s.campaignId === campaignId && s.order >= order 
            ? { ...s, order: s.order + 1 } 
            : s
        )
      );
  ...
  const deleteCampaignStep = (stepId: string) => {
    ...
    if (affectedSteps.length > 0) {
      setCampaignSteps(prev => 
        prev.map(s => 
          s.campaignId === campaignId && s.order > step.order 
            ? { ...s, order: s.order - 1 } 
            : s
        )
      );
  ...
  const updateEmailLogStatus = (id: string, status: EmailLog['status'], details?: Partial<EmailLog>) => {
  ...
  const sendTestEmail = async (templateId: string, to: string, candidateId?: number, jobId?: number) => {
    // In a real app, you would:
  ...
  const processEmailTriggers = async (trigger: EmailTrigger, candidateId: number, specifics?: any) => {
    // In a real app, you would:
