...
// Create the context
const AdvancedDashboardContext = createContext<AdvancedDashboardContextProps | undefined>(
  undefined
);
...
const mockReportData: Record<string, any[]> = {
  'report-1': [
    ...
    },
    {
      department: 'HR',
      positions_filled: 3,
      avg_time_to_fill: 20.1,
      cost_per_hire: 2900,
      top_source: 'Company Website',
    },
    {
      department: 'Finance',
      positions_filled: 4,
      avg_time_to_fill: 35.8,
      cost_per_hire: 4500,
      top_source: 'LinkedIn',
    },
  ],
  'report-2': [
    {
      department: 'Engineering',
      advertising_costs: 12000,
      agency_fees: 25000,
      internal_costs: 18000,
      total: 55000,
    },
    {
      department: 'Sales',
      advertising_costs: 8500,
      agency_fees: 15000,
      internal_costs: 12000,
      total: 35500,
    },
    {
      department: 'Marketing',
      advertising_costs: 7000,
      agency_fees: 12000,
      internal_costs: 9500,
      total: 28500,
    },
...
export const AdvancedDashboardProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const defaultDashboard = dashboards.find(
      (d) => d.isDefault && d.allowedRoles.includes(userRole)
    );
    if (defaultDashboard) {
  ...
  const updateDashboard = useCallback(
    async (id: string, updates: Partial<Dashboard>): Promise<Dashboard> => {
      setIsLoading(true);
      setError(null);
  ...
  const addWidget = useCallback(
    async (dashboardId: string, widgetData: Omit<Widget, 'id'>): Promise<Widget> => {
      setIsLoading(true);
      setError(null);
  ...
  const updateWidget = useCallback(
    async (dashboardId: string, widgetId: string, updates: Partial<Widget>): Promise<Widget> => {
      setIsLoading(true);
      setError(null);
  ...
  // Report operations
  const getReportsForRole = useCallback(
    (role: string): Report[] => {
  ...
  const runReport = useCallback(
    async (reportId: string, parameters?: Record<string, any>): Promise<any[]> => {
      setIsLoading(true);
      setError(null);
  ...
  const fetchWidgetData = useCallback(
    async (widgetId: string, dashboardId: string): Promise<any> => {
      setIsLoading(true);
      setError(null);
  ...
  const refreshWidget = useCallback(
    async (widgetId: string, dashboardId: string): Promise<void> => {
      try {
...
export const useAdvancedDashboard = (): AdvancedDashboardContextProps => {
  ...
  if (context === undefined) {
    console.warn(
      'useAdvancedDashboard used outside of AdvancedDashboardProvider, returning fallback data'
    );
