...
interface ReferralContextType {
  ...
  // Program operations
  createProgram: (program: Omit<ReferralProgram, 'id' | 'createdAt' | 'updatedAt'>) => Promise<ReferralProgram>;
  ...
  // Referral operations
  createReferral: (referral: Omit<Referral, 'id' | 'submissionDate' | 'updatedAt' | 'rewardsIssued'>) => Promise<Referral>;
  updateReferralStatus: (id: string, status: Referral['status'], notes?: string) => Promise<Referral>;
...
export const ReferralProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const createProgram = useCallback(async (
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to create referral program';
  ...
  const updateProgram = useCallback(async (
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to update referral program';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setIsLoading(false);
  ...
  const deleteProgram = useCallback(async (id: string): Promise<boolean> => {
    ...
    try {
      ...
      const activeReferrals = referrals.some(
        referral => referral.programId === id && 
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to delete referral program';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setIsLoading(false);
  ...
  const toggleProgramActive = useCallback(async (id: string): Promise<ReferralProgram> => {
    setIsLoading(true);
    setError(null);
  ...
  const updateReferralStatus = useCallback(async (
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to update referral status';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setIsLoading(false);
  ...
  const addReferralReward = useCallback(async (
    id: string,
    milestone: string,
    amount: number
  ): Promise<Referral> => {
  ...
  // Analytics and reporting
  const getReferralsByProgram = useCallback((programId: string): Referral[] => {
    return referrals.filter(referral => referral.programId === programId);
  }, [referrals]);

  const getReferralsByReferrer = useCallback((referrerId: string): Referral[] => {
  ...
  const getReferralProgramStats = useCallback((programId: string) => {
