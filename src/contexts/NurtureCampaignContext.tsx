...
import { NurtureCampaign, NurtureStep, CandidateRelationship } from '../models/types';
...
const sampleCampaigns: NurtureCampaign[] = [
  {
    ...
    steps: [
      ...
      },
      {
        ...
        type: 'LinkedIn',
        content:
          'Hi {{candidate.firstName}}, I noticed your profile and wanted to connect regarding opportunities at our company.',
        delayDays: 14,
        condition: {
          previousStepOutcome: 'None',
        },
...
export const NurtureCampaignProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const createCampaign = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to create nurture campaign';
  ...
  const updateCampaign = useCallback(
    async (id: string, updates: Partial<NurtureCampaign>): Promise<NurtureCampaign> => {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to update nurture campaign';
        setError(errorMessage);
        throw new Error(errorMessage);
      } finally {
        setIsLoading(false);
  ...
  const deleteCampaign = useCallback(
    async (id: string): Promise<boolean> => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to delete nurture campaign';
        setError(errorMessage);
        throw new Error(errorMessage);
      } finally {
        setIsLoading(false);
  ...
  // Step operations
  const addStepToCampaign = useCallback(
    async (
      campaignId: string,
      step: Omit<NurtureStep, 'id' | 'campaignId'>
    ): Promise<NurtureStep> => {
  ...
  const removeStep = useCallback(
    async (campaignId: string, stepId: string): Promise<boolean> => {
      setIsLoading(true);
      setError(null);
  ...
  const assignCampaignToTalentPool = useCallback(
    async (campaignId: string, talentPoolId: string): Promise<boolean> => {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        const candidatesInPool =
          talentPools.find((pool) => pool.id === talentPoolId)?.candidateIds.length || 0;
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to assign campaign to talent pool';
        setError(errorMessage);
        throw new Error(errorMessage);
      } finally {
        setIsLoading(false);
  ...
  const unassignCampaignFromTalentPool = useCallback(
    async (campaignId: string, talentPoolId: string): Promise<boolean> => {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to unassign campaign from talent pool';
        setError(errorMessage);
        throw new Error(errorMessage);
      } finally {
        setIsLoading(false);
  ...
  const getCampaignStats = useCallback(
    async (id: string): Promise<NurtureCampaign['stats']> => {
      setIsLoading(true);
      setError(null);
  ...
  const getCampaignsByTalentPool = useCallback(
    (talentPoolId: string): NurtureCampaign[] => {
  ...
  // Campaign preview and testing
  const previewCampaignStep = useCallback(
    async (stepId: string, campaignId: string): Promise<{ subject?: string; content: string }> => {
      setIsLoading(true);
      setError(null);
  ...
  const sendTestMessage = useCallback(
    async (stepId: string, campaignId: string, recipientEmail: string): Promise<boolean> => {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        console.log(
          `Test message sent to ${recipientEmail} for step ${stepId} of campaign ${campaignId}`
        );
