...
interface ResumeParsingContextType {
  ...
  detectEmploymentGaps: (resume: ParsedResume) => {
    hasGaps: boolean;
    gapPeriods: string[];
...
export const ResumeParsingProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [questionnaires, setQuestionnaires] = useState<ScreeningQuestionnaire[]>(
    mockQuestionnaires || []
  );
  ...
  const screenCandidate = async (
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const requiredSkillsMissing = requiredSkills.filter(
        ...
        const relevantExperience = parsedResume.parsedData.experience.filter(
          (exp) =>
        ...
        const hasRelevantDegree = parsedResume.parsedData.education.some(
          (edu) =>
        ...
        const screeningResult: ScreeningResult = {
          ...
          overallScore: Math.round(
            (skillMatchScore + experienceMatchScore + (hasRelevantDegree ? 100 : 50)) / 3
          ),
          skillMatch: {
    ...
    parsedResumes.forEach((resume) => {
      ...
      if (similarCandidates.length > 0) {
        ...
        duplicates.push({
          ...
          similarity:
            similarCandidates.reduce((sum, c) => sum + c.similarity, 0) / similarCandidates.length,
        });
  ...
  const calculateResumeSimilarity = (resume1: ParsedResume, resume2: ParsedResume): number => {
    ...
    const skillsSimilarity = commonSkills / Math.max(1, (skillsSet1.size + skillsSet2.size) / 2);
  ...
  const calculateCandidateFit = async (
    ...
    const result = screeningResults.find((r) => r.candidateId === candidateId && r.jobId === jobId);
    ...
    if (result.experienceMatch.yearsOfRelevantExperience < 2) {
      flags.push(
        `Limited relevant experience (${result.experienceMatch.yearsOfRelevantExperience} years)`
      );
    }
  ...
  return (
    <ResumeParsingContext.Provider value={contextValue}>{children}</ResumeParsingContext.Provider>
  );
};
