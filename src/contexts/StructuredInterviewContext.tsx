...
const mockQuestions: InterviewQuestion[] = [
  {
    ...
    expectedAnswer: 'Both are hooks for optimization. useMemo memoizes a computed value and only recalculates it when dependencies change. useCallback memoizes a function definition to prevent unnecessary re-renders of child components that receive the function as a prop.',
    scoringRubric: [
      {
        criteria: 'Technical accuracy',
        weightage: 40,
        excellentResponse: 'Explains both hooks accurately, including their purpose, usage scenarios, and performance implications',
      ...
      },
      {
        ...
        weightage: 30,
        excellentResponse: 'Demonstrates deep understanding of React's rendering behavior and optimization principles',
  ...
  {
    id: '2',
    question: 'Describe a time when you had to give difficult feedback to a team member. How did you approach it and what was the outcome?',
    category: 'behavioral',
    difficultyLevel: 'intermediate',
    expectedAnswer: 'A strong answer would demonstrate the candidate\'s communication skills, empathy, and ability to handle difficult conversations. They should describe a specific situation, their thoughtful approach to providing feedback, and a positive or constructive outcome.',
    scoringRubric: [
      {
        criteria: 'Communication approach',
        weightage: 35,
        excellentResponse: 'Describes a structured, empathetic approach with specific techniques used',
        goodResponse: 'Explains a thoughtful approach with consideration for the recipient\'s feelings',
      ...
      },
      {
        ...
        weightage: 35,
        excellentResponse: 'Demonstrates high self-awareness and reflection on own communication style',
  ...
  },
  {
    ...
    expectedAnswer: 'A height-balanced binary tree is a binary tree in which the depth of the two subtrees of every node never differs by more than 1. The solution involves writing a recursive function that calculates the height of each subtree and checks if the difference is within the allowed range.',
    scoringRubric: [
      {
...
const mockInterviewKits: InterviewKit[] = [
  {
    ...
    description: 'A structured technical interview kit for evaluating frontend developer candidates, focusing on React, JavaScript, and web fundamentals.',
    ...
    instructions: 'This interview should assess the candidate\'s technical knowledge and problem-solving abilities. Start with a brief introduction, then proceed with the required questions. If time permits, ask the optional questions. Allow time for the candidate to ask questions at the end.',
    createdBy: '2', // David Lee
  ...
  },
  {
    ...
    description: 'A comprehensive behavioral interview kit for evaluating leadership qualities, communication skills, and cultural fit.',
    ...
    instructions: 'Focus on assessing the candidate\'s leadership style, communication skills, and past experiences. Use follow-up questions to dig deeper into their responses. Look for specific examples and outcomes in their answers.',
    createdBy: '4', // Michael Johnson
...
interface StructuredInterviewContextType {
  ...
  updateQuestion: (id: string, updates: Partial<InterviewQuestion>) => InterviewQuestion | undefined;
  ...
  createScheduleTemplate: (template: Omit<InterviewScheduleTemplate, 'id' | 'createdAt' | 'updatedAt'>) => InterviewScheduleTemplate;
  updateScheduleTemplate: (id: string, updates: Partial<InterviewScheduleTemplate>) => InterviewScheduleTemplate | undefined;
...
// Create the context
const StructuredInterviewContext = createContext<StructuredInterviewContextType | undefined>(undefined);
...
export const StructuredInterviewProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [scheduleTemplates, setScheduleTemplates] = useState<InterviewScheduleTemplate[]>(mockScheduleTemplates);
  
  // Question management functions
  const addQuestion = (questionData: Omit<InterviewQuestion, 'id' | 'createdAt' | 'updatedAt'>): InterviewQuestion => {
  ...
  const updateQuestion = (id: string, updates: Partial<InterviewQuestion>): InterviewQuestion | undefined => {
  ...
  const deleteQuestion = (id: string): boolean => {
    ...
    const isUsedInKit = interviewKits.some(kit => 
      kit.questions.some(q => q.questionId === id)
    );
  ...
  const createInterviewKit = (kitData: Omit<InterviewKit, 'id' | 'createdAt' | 'updatedAt'>): InterviewKit => {
    const timestamp = new Date().toISOString();
  ...
  const updateInterviewKit = (id: string, updates: Partial<InterviewKit>): InterviewKit | undefined => {
  ...
  const createScheduleTemplate = (templateData: Omit<InterviewScheduleTemplate, 'id' | 'createdAt' | 'updatedAt'>): InterviewScheduleTemplate => {
    const timestamp = new Date().toISOString();
  ...
  const updateScheduleTemplate = (id: string, updates: Partial<InterviewScheduleTemplate>): InterviewScheduleTemplate | undefined => {
