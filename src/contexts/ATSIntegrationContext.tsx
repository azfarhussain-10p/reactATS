...
interface ATSIntegrationContextType {
  ...
  shareDataWith: (
    sourceComponent: ATSComponentType,
    targetComponent: ATSComponentType,
    data: any
  ) => void;
...
export const ATSIntegrationProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  try {
    ...
    console.warn(
      'Router context not available for ATSIntegrationProvider. Navigation will be limited.'
    );
  ...
  return <ATSIntegrationContext.Provider value={value}>{children}</ATSIntegrationContext.Provider>;
};
