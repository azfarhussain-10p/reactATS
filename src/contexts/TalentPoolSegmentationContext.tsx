...
interface TalentPoolSegmentationContextType {
  ...
  createSegment: (segment: Omit<TalentSegment, 'id' | 'createdAt' | 'updatedAt' | 'candidateCount'>) => TalentSegment;
  ...
  forecastSegmentGrowth: (segmentId: string, months: number) => Promise<{ date: string, count: number }[]>;
}
...
// Create context
const TalentPoolSegmentationContext = createContext<TalentPoolSegmentationContextType | undefined>(undefined);
...
export const TalentPoolSegmentationProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [segmentMetrics, setSegmentMetrics] = useState<Record<string, SegmentationMetric>>(sampleSegmentMetrics);

  // Create a new segment
  const createSegment = (segmentData: Omit<TalentSegment, 'id' | 'createdAt' | 'updatedAt' | 'candidateCount'>): TalentSegment => {
  ...
  const mergeSegments = (segmentIds: string[], newSegmentName: string): TalentSegment | null => {
    ...
    segmentsToMerge.forEach(segment => {
      segment.criteria.forEach(criteria => {
        if (!allCriteria.some(c => c.field === criteria.field && c.operator === criteria.operator)) {
    ...
    const totalCandidates = segmentsToMerge.reduce((total, segment) => total + segment.candidateCount, 0);
    ...
    setSegments([...segments, newSegment]);
    announce(`Created merged talent segment: ${newSegment.name} with ${totalCandidates} candidates`);
    return newSegment;
  };
  ...
  const forecastSegmentGrowth = async (segmentId: string, months: number): Promise<{ date: string, count: number }[]> => {
    // Simulate API call
    await new Promise(resolve => setTimeout(resolve, 1000));
...
export const useTalentPoolSegmentation = () => {
  ...
  if (context === undefined) {
    throw new Error('useTalentPoolSegmentation must be used within a TalentPoolSegmentationProvider');
  }
  return context;
}; 