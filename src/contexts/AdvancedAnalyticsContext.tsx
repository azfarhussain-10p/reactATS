...
interface AdvancedAnalyticsContextType {
  ...
  updateVisualization: (
    id: string,
    updates: Partial<DataVisualization>
...
export const AdvancedAnalyticsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [predictiveModels, setPredictiveModels] =
    useState<PredictiveModel[]>(samplePredictiveModels);
  ...
  const runTrendAnalysis = async (
    ...
    const now = new Date();
    const points =
      period === 'weekly' ? 12 : period === 'monthly' ? 12 : period === 'quarterly' ? 8 : 5;
  ...
  const runSegmentAnalysis = async (segmentBy: string): Promise<SegmentAnalysis> => {
    ...
    const segmentNames =
      segmentBy === 'department'
        ...
        : segmentBy === 'seniority'
          ? ['Junior', 'Mid-level', 'Senior', 'Lead', 'Executive']
          : segmentBy === 'location'
