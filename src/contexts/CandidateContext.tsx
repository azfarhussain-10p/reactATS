...
export const CandidateProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const filterCandidates = (filters: CandidateFilters) => {
    return candidates.filter((candidate) => {
      ...
      if (filters.skills && filters.skills.length > 0) {
        const candidateSkillNames = candidate.professionalInfo.skills.map((skill) =>
      ...
      if (
        ...
        candidate.personalInfo.willingToRelocate !== filters.willingToRelocate
      ) {
        return false;
      }
      ...
      if (
        ...
        candidate.professionalInfo.totalYearsOfExperience < filters.minYearsOfExperience
      ) {
        return false;
      }
      ...
      if (
        filters.location &&
        !candidate.personalInfo.location.toLowerCase().includes(filters.location.toLowerCase())
      ) {
        return false;
      }
  ...
  const deleteSkill = (candidateId: number, skillId: string) => {
    ...
    const filteredSkills = candidate.professionalInfo.skills.filter(
  ...
  const deleteCommunication = (candidateId: number, communicationId: string) => {
    ...
    const filteredCommunications = candidate.communications.filter(
  ...
  const deleteJobApplication = (candidateId: number, applicationId: string) => {
    ...
    const filteredApplications = candidate.jobApplications.filter(
  ...
  const getCandidatesByTag = (tag: string) => {
    return candidates.filter((candidate) =>
      candidate.professionalInfo.tags.some((t) => t.toLowerCase() === tag.toLowerCase())
    );
  };
  ...
  const searchCandidates = (query: string) => {
    ...
    return candidates.filter((candidate) => {
      const fullName =
        `${candidate.personalInfo.firstName} ${candidate.personalInfo.lastName}`.toLowerCase();
