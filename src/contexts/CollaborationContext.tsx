...
const mockNotes: CollaborativeNote[] = [
  {
    id: '1',
    candidateId: 1,
    content: 'Candidate showed strong problem-solving skills during the technical interview. @David Lee might want to focus on system design questions in the next round.',
  ...
  },
  {
    id: '2',
    candidateId: 1,
    content: 'Verified candidate\'s references. All positive feedback about teamwork and communication skills.',
...
const mockMessages: Message[] = [
  {
    ...
    content: 'Hi team, I\'ve scheduled the technical interview for John Doe on Thursday at 2pm. @Emily Chen, can you join?',
...
const mockTasks: CollaborationTask[] = [
  {
    ...
    description: 'Coordinate with the engineering team to schedule a technical interview for John Doe',
...
interface CollaborationContextType {
  ...
  updateTeamMemberNotifications: (id: string, notifications: { type: string, channel: string }[]) => boolean;
  ...
  // Task management
  createTask: (task: Omit<CollaborationTask, 'id' | 'createdAt' | 'updatedAt' | 'notifications'>) => CollaborationTask;
  ...
  createDocument: (document: Omit<SharedDocument, 'id' | 'createdAt' | 'versionHistory'>) => SharedDocument;
  updateDocument: (id: string, updates: { content: string, updatedBy: string }) => SharedDocument | undefined;
...
export const CollaborationProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const updateTeamMemberNotifications = (id: string, notifications: { type: string, channel: string }[]): boolean => {
  ...
  const updateNote = (id: string, updates: Partial<CollaborativeNote>): CollaborativeNote | undefined => {
  ...
  const getMessagesForUser = (userId: string): Message[] => {
    return messages.filter(
      message => message.receiverIds.includes(userId) || message.senderId === userId
    ).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
  };
  
  // Task management functions
  const createTask = (taskData: Omit<CollaborationTask, 'id' | 'createdAt' | 'updatedAt' | 'notifications'>): CollaborationTask => {
  ...
  const updateTask = (id: string, updates: Partial<CollaborationTask>): CollaborationTask | undefined => {
  ...
  const getVoteSummary = (candidateId: number): {
    strongHire: number;
    hire: number;
    neutral: number;
    reject: number;
  ...
  const createDocument = (documentData: Omit<SharedDocument, 'id' | 'createdAt' | 'versionHistory'>): SharedDocument => {
    const timestamp = new Date().toISOString();
  ...
  const updateDocument = (id: string, updates: { content: string, updatedBy: string }): SharedDocument | undefined => {
