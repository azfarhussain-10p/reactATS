...
interface ComplianceContextType {
  ...
  checkDataRetention: (dataType: keyof typeof DATA_RETENTION_POLICIES, dateCreated: string) => boolean;
  
  // Consent management
  consentRecords: ConsentRecord[];
  recordConsent: (candidateId: number, consentType: ConsentType, consentGiven: boolean, additionalNotes?: string) => ConsentRecord;
  ...
  createDataRequest: (requestType: 'access' | 'deletion' | 'correction' | 'portability', candidateId: number, candidateName: string, candidateEmail: string, requestNotes?: string) => DataRequest;
  updateDataRequestStatus: (requestId: string, status: 'pending' | 'processing' | 'completed' | 'denied', processingNotes?: string) => DataRequest | null;
...
export const ComplianceProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [retentionPeriods, setRetentionPeriods] = useState<Record<string, number>>(mockRetentionPeriods);
  ...
  const setRetentionPeriod = (dataType: keyof typeof DATA_RETENTION_POLICIES, days: number): void => {
  ...
  const checkDataRetention = (dataType: keyof typeof DATA_RETENTION_POLICIES, dateCreated: string): boolean => {
  ...
  const updateDataRequestStatus = (
    ...
    const updatedRequest: DataRequest = {
      ...
      ...(status === 'completed' || status === 'denied' ? { completionDate: new Date().toISOString() } : {})
    };
  ...
  const recordEEOData = (jobId: number, demographicData: any): EEORecord => {
    ...
    addAuditLog(
      'EEO_DATA_RECORDED',
      `EEO data recorded for job #${jobId}`,
      { jobId }
    );
  ...
  const getEEOStatsByJob = (jobId: number): any => {
    ...
    jobRecords.forEach(record => {
      ...
      if (record.demographicData.gender) {
        stats.gender[record.demographicData.gender] = (stats.gender[record.demographicData.gender] || 0) + 1;
      }
      
      // Race stats
      if (record.demographicData.race) {
        stats.race[record.demographicData.race] = (stats.race[record.demographicData.race] || 0) + 1;
      }
      
      // Disability stats
      if (record.demographicData.disability) {
        stats.disability[record.demographicData.disability] = (stats.disability[record.demographicData.disability] || 0) + 1;
      }
      
      // Veteran status stats
      if (record.demographicData.veteranStatus) {
        stats.veteranStatus[record.demographicData.veteranStatus] = (stats.veteranStatus[record.demographicData.veteranStatus] || 0) + 1;
      }
    });
  ...
  const anonymizeCandidate = (candidateId: number): boolean => {
    ...
    addAuditLog(
      ...
      `Candidate #${candidateId} data was anonymized`,
      { candidateId }
    );
  ...
  const addAuditLog = (eventType: string, description: string, metadata?: any): ComplianceAuditLog => {
