...
const mockCategories: QuestionCategory[] = [
  { id: '1', name: 'Technical Skills', description: 'Questions to assess technical knowledge and abilities' },
  { id: '2', name: 'Problem Solving', description: 'Questions to evaluate analytical and problem-solving abilities' },
  { id: '3', name: 'Communication', description: 'Questions to assess verbal and written communication skills' },
  { id: '4', name: 'Leadership', description: 'Questions to evaluate leadership potential and experience' },
  { id: '5', name: 'Cultural Fit', description: 'Questions to assess alignment with company values and culture' }
];
...
const mockRubricItems: RubricItem[] = [
  { id: 'rub-1', criteriaId: '1', score: 1, description: 'Little to no relevant technical knowledge' },
  { id: 'rub-2', criteriaId: '1', score: 2, description: 'Basic understanding but significant gaps' },
  ...
  { id: 'rub-7', criteriaId: '2', score: 3, description: 'Adequately solves problems with some guidance' },
  { id: 'rub-8', criteriaId: '2', score: 5, description: 'Exceptional problem-solving with innovative approaches' },
  { id: 'rub-9', criteriaId: '3', score: 1, description: 'Poor communication, difficult to understand' },
  { id: 'rub-10', criteriaId: '3', score: 3, description: 'Clear communication with occasional issues' },
  { id: 'rub-11', criteriaId: '3', score: 5, description: 'Exceptional communicator, articulate and engaging' }
];
...
const mockQuestions: InterviewQuestion[] = [
  {
    ...
    categoryId: '1',
    difficultyLevelId: '3',
    expectedAnswer: 'A good answer should compare both technologies, highlighting that REST is a style of API design using standard HTTP methods while GraphQL is a query language for APIs allowing clients to request exactly what they need.',
    rubricItems: ['rub-1', 'rub-3', 'rub-5'],
    timeGuidelineMinutes: 10,
    followUpQuestions: ['What are the pros and cons of each approach?', 'When would you choose one over the other?']
  },
  {
    id: '2',
    ...
    categoryId: '3',
    difficultyLevelId: '2',
    expectedAnswer: 'Looking for examples that demonstrate ability to translate technical concepts, adapt communication style, and ensure understanding.',
  ...
  },
  {
    ...
    categoryId: '2',
    difficultyLevelId: '5',
    expectedAnswer: 'Candidate should discuss load balancing, caching strategies, database optimization, horizontal scaling, and potential bottlenecks.',
    rubricItems: ['rub-6', 'rub-7', 'rub-8'],
    timeGuidelineMinutes: 15,
    followUpQuestions: ['How would you monitor this system?', 'What failure scenarios should we prepare for?']
  }
];
...
const mockScheduleTemplates: InterviewScheduleTemplate[] = [
  {
    ...
    stages: [
      mockInterviewStages[0],
      mockInterviewStages[1],
      mockInterviewStages[2]
    ],
...
interface StructuredInterviewKitContextType {
  ...
  createScheduleTemplate: (template: Omit<InterviewScheduleTemplate, 'id' | 'createdAt' | 'updatedAt'>) => void;
...
const StructuredInterviewKitContext = createContext<StructuredInterviewKitContextType | undefined>(undefined);
...
export const useStructuredInterviewKit = (): StructuredInterviewKitContextType => {
  ...
  if (!context) {
    throw new Error('useStructuredInterviewKit must be used within a StructuredInterviewKitProvider');
  }
  return context;
};
...
export const StructuredInterviewKitProvider = ({ children }: StructuredInterviewKitProviderProps) => {
  ...
  const createScheduleTemplate = (template: Omit<InterviewScheduleTemplate, 'id' | 'createdAt' | 'updatedAt'>) => {
  ...
  const updateScheduleTemplate = (id: string, updatedTemplate: Partial<InterviewScheduleTemplate>) => {
