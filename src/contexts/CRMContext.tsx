...
interface CRMContextType {
  ...
  createTalentPool: (pool: Omit<TalentPool, 'id' | 'createdAt' | 'updatedAt' | 'candidateCount'>) => TalentPool;
  ...
  recordInteraction: (interaction: Omit<CandidateInteraction, 'id' | 'date'>) => CandidateInteraction;
  getCandidateInteractions: (candidateId: string) => CandidateInteraction[];
  updateInteraction: (id: string, updates: Partial<CandidateInteraction>) => CandidateInteraction | null;
  deleteInteraction: (id: string) => boolean;
  
  // Nurture Campaigns
  createCampaign: (campaign: Omit<NurtureCampaign, 'id' | 'createdAt' | 'updatedAt' | 'metrics'>) => NurtureCampaign;
  ...
  // Event Management
  createEvent: (event: Omit<RecruitmentEvent, 'id' | 'registeredCandidates' | 'attendees' | 'metrics'>) => RecruitmentEvent;
  ...
  // Alumni Network
  addAlumnus: (alumnus: Omit<AlumnusProfile, 'id' | 'relationships' | 'engagementScore'>) => AlumnusProfile;
  updateAlumnusProfile: (id: string, updates: Partial<AlumnusProfile>) => AlumnusProfile | null;
  trackAlumnusInteraction: (id: string, interaction: Omit<CandidateInteraction, 'id' | 'candidateId' | 'date'>) => CandidateInteraction;
  getAlumniByDepartment: (department: string) => AlumnusProfile[];
  
  // External Integrations
  configureExternalIntegration: (integration: Omit<ExternalCRMIntegration, 'id' | 'lastSync'>) => ExternalCRMIntegration;
  updateIntegrationSettings: (id: string, settings: Record<string, any>) => ExternalCRMIntegration | null;
...
export const CRMProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [nurtureCampaigns, setNurtureCampaigns] = useState<NurtureCampaign[]>(defaultNurtureCampaigns);
  ...
  const createTalentPool = (pool: Omit<TalentPool, 'id' | 'createdAt' | 'updatedAt' | 'candidateCount'>) => {
  ...
  const createNurtureCampaign = (campaign: Omit<NurtureCampaign, 'id' | 'createdAt' | 'updatedAt' | 'stats'>) => {
  ...
  const updateCampaignStep = (campaignId: string, stepId: string, updates: Partial<NurtureStep>) => {
    const campaign = nurtureCampaigns.find(c => c.id === campaignId);
  ...
  const getScoreForEngagementLevel = (level: EngagementLevel): number => {
    switch (level) {
      case 'Very Hot': return 100;
      case 'Hot': return 75;
      case 'Warm': return 50;
      case 'Cold': return 25;
      case 'Not Interested': return 0;
      default: return 0;
    }
  };
  ...
  // Candidate Interactions
  const recordInteraction = (interaction: Omit<CandidateInteraction, 'id' | 'date'>): CandidateInteraction => {
  ...
  // External Integrations
  const syncWithExternalCRM = async (id: string): Promise<{ success: boolean; message: string }> => {
  ...
  const value: CRMContextType = {
    ...
    getCampaignStats: (id) => nurtureCampaigns.find(c => c.id === id)?.metrics || { 
    ...
    getEventStats: (id) => recruitmentEvents.find(e => e.id === id)?.metrics || {
      registrationCount: 0,
      attendanceCount: 0,
      hiringConversions: 0
    },
