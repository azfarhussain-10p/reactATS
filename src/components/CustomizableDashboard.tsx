...
const ChartWidget: React.FC<{
  ...
  const renderChartIcon = () => {
    switch (type) {
      ...
      case 'metric':
        return <NumbersIcon fontSize="large" />;
      default:
        return <BarChartIcon fontSize="large" />;
    }
  };
  ...
  return (
    <Paper sx={{ p: 2, height: getHeight(), display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
        ...
        <Button size="small" onClick={handleMenuOpen} sx={{ minWidth: 'auto' }}>
          <MoreVertIcon />
        </Button>
  ...
  if (dashboardTabs.length === 0) {
    return (
      <Box sx={{ p: 3, textAlign: 'center' }}>
        ...
        {canEdit && (
          <Button variant="contained" startIcon={<AddIcon />} onClick={handleAddDashboard}>
            Create Dashboard
          </Button>
        )}
      </Box>
    ...
    <Box sx={{ p: 2 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        ...
        <Box>
          ...
          <Button sx={{ ml: 1 }} startIcon={<DownloadIcon />}>
            Export
          </Button>
          <Button sx={{ ml: 1 }} startIcon={<ShareIcon />}>
            Share
          </Button>
          {canEdit && (
            <Button sx={{ ml: 1 }} startIcon={<SettingsIcon />}>
              Settings
            </Button>
          )}
        </Box>
