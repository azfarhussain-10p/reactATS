    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
          ...
          <Container maxWidth="md">
            <Paper sx={{ p: 3, mb: 3 }}>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                Set up conditions to control when fields are shown or hidden based on user responses.
              </Typography>
              
              {activeForm.conditions.length === 0 ? (
                <Box sx={{ textAlign: 'center', py: 4, borderRadius: 1, bgcolor: 'background.paper' }}>
                  <Typography variant="body1" sx={{ mb: 2 }}>No conditions have been created yet.</Typography>
                  <Button 
                    variant="contained" 
                    startIcon={<AddIcon />}
                    onClick={() => {
                ...
                <>
                  ...
                  {activeForm.conditions.map((condition) => {
                    ...
                    const field = activeForm.pages.flatMap(p => p.fields).find(f => f.id === condition.fieldId);
                    ...
                    return (
                      <Paper key={condition.id} sx={{ p: 2, mb: 2, border: '1px solid', borderColor: 'divider' }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                          ...
                          <Box>
                            <IconButton size="small" onClick={() => deleteCondition(activeForm.id, condition.id)}>
                          ...
                          <Grid item xs={12} md={4}>
                            <FormControl fullWidth size="small">
                              ...
                              <Select
                                ...
                                label="Field"
                                onChange={(e) => updateCondition(activeForm.id, condition.id, { fieldId: e.target.value })}
                                MenuProps={menuProps}
                              >
                                {activeForm.pages.flatMap(page => page.fields).map(field => (
                                  <MenuItem key={field.id} value={field.id} sx={{ color: 'text.primary' }}>
                          ...
                          <Grid item xs={12} md={3}>
                            <FormControl fullWidth size="small">
                              ...
                              <Select
                                ...
                                label="Operator"
                                onChange={(e) => updateCondition(activeForm.id, condition.id, { operator: e.target.value })}
                                MenuProps={menuProps}
                              >
                                <MenuItem value="equals" sx={{ color: 'text.primary' }}>Equals</MenuItem>
                                <MenuItem value="notEquals" sx={{ color: 'text.primary' }}>Not Equals</MenuItem>
                                <MenuItem value="contains" sx={{ color: 'text.primary' }}>Contains</MenuItem>
                                <MenuItem value="greaterThan" sx={{ color: 'text.primary' }}>Greater Than</MenuItem>
                                <MenuItem value="lessThan" sx={{ color: 'text.primary' }}>Less Than</MenuItem>
                                <MenuItem value="isAnyOf" sx={{ color: 'text.primary' }}>Is Any Of</MenuItem>
                              </Select>
                            </FormControl>
                          </Grid>
                          ...
                          <Grid item xs={12} md={5}>
                            {condition.operator === 'isAnyOf' ? (
                              ...
                              <TextField
                                ...
                                onChange={(e) => updateCondition(activeForm.id, condition.id, { value: e.target.value })}
                              />
                            )}
                          </Grid>
                        </Grid>
            ...
            <Paper sx={{ p: 3 }}>
              <Typography variant="h6" gutterBottom>Apply Conditions to Fields</Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                        ...
                        <TableBody>
                          {page.fields.map((field) => (
                            <TableRow key={field.id}>
                              <TableCell>{field.label || `Field ${field.id.slice(0, 4)}`}</TableCell>
                              <TableCell>{field.type}</TableCell>
                              <TableCell>
                                {field.conditionId ? (
                                  // Find and display the condition details
                                  (() => {
                                    const condition = activeForm.conditions.find(c => c.id === field.conditionId);
                                    const triggerField = condition ? 
                                      activeForm.pages.flatMap(p => p.fields).find(f => f.id === condition.fieldId) : null;
                                    
                                    return condition ? (
                                      <Typography variant="body2">
                                        When {triggerField?.label || 'field'} {condition.operator} {condition.value || condition.values?.join(', ')}
                                      </Typography>
                                    ) : 'Invalid condition';
                                  })()
                                ) : (
                              ...
                              <TableCell>
                                <FormControl fullWidth size="small">
                                  <InputLabel id="condition-select-label">Apply Condition</InputLabel>
                                  <Select
                                    ...
                                    onChange={(e) => {
                                      ...
                                      if (value) {
                                        updateField(activeForm.id, page.id, field.id, { conditionId: value });
                                      } else {
                                        // Remove condition
                                  ...
                                  >
                                    <MenuItem value="" sx={{ color: 'text.primary' }}>No condition (always show)</MenuItem>
                                    {activeForm.conditions.map(cond => (
                                      <MenuItem key={cond.id} value={cond.id} sx={{ color: 'text.primary' }}>
                                        {(() => {
                                          const triggerField = activeForm.pages.flatMap(p => p.fields)
      ...
      {currentTab === 2 && (
        <Box sx={{ flexGrow: 1, p: 3, overflow: 'auto' }}>
          <Container maxWidth="md">
            <Paper sx={{ p: 3, mb: 3 }}>
              <Typography variant="h6" gutterBottom>Form Submissions</Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
              ...
              {activeForm.submissions && activeForm.submissions.length > 0 ? (
                ...
                <Box sx={{ textAlign: 'center', py: 4, borderRadius: 1, bgcolor: 'background.paper' }}>
                  <Typography variant="body1" sx={{ mb: 2 }}>No submissions yet.</Typography>
            ...
            <Paper sx={{ p: 3 }}>
              ...
              <FormControl fullWidth sx={{ mb: 2 }}>
                <InputLabel id="default-status-label">Default Status for New Submissions</InputLabel>
                <Select
                  ...
                  MenuProps={menuProps}
                >
                  <MenuItem value="new" sx={{ color: 'text.primary' }}>New</MenuItem>
                  <MenuItem value="review" sx={{ color: 'text.primary' }}>Needs Review</MenuItem>
                  <MenuItem value="approved" sx={{ color: 'text.primary' }}>Approved</MenuItem>
                </Select>
              </FormControl>
